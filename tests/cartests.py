...
class CarsTests(unittest.TestCase):
  ...
  def test_add(self):
    car1 = self.cars.add('Bon Voyage', 'red')
    car2 = self.cars.add('VW Jetta', 'green')
  ...
  def test_get_not_empty(self):
    self.cars.add('Bon Voyage', 'red')
    self.cars.add('VW Jetta', 'green')
    self.assertEqual(len(self.cars.get()), 2, "cars not retrieved")
  
  def test_get_copy(self):
    cars = self.cars.get()
    cars.append(Car(1, 'Random House', 'strange color'))
    self.assertEqual(len(self.cars.get()), 0, "cars not retrieved")

  def test_delete(self):
    car = self.cars.add('Bon Voyage', 'red')
    self.cars.add('VW Jetta', 'green')
